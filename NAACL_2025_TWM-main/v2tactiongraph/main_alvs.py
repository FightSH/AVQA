...
from v2tactiongraph.dataloaders.dataloader_msrvtt_caption import MSRVTT_ALVS_DataLoader
...
def get_args(description='CaptionGenerator'):
    ...
    parser.add_argument('--no_skip', action='store_true',
                        help="Whether to remove the additional skip connection in the gnn model")
    parser.add_argument('--last_average', action='store_true',
                        help="Whether to use average or concatenation in the last layer of gnn model")
    parser.add_argument('--no_beta_transformer', action='store_true',
                        help="Whether to not use beta on the transformer model")
    ...
    parser.add_argument('--target_metric', type=str, default="CIDEr",
                        help="Target metric which is used to select the best model.")
...
def init_model_theta_1(args, device, n_gpu, local_rank):
    ...
    caption_generator_cache_dir = args.cache_dir if args.cache_dir else os.path.join(str(PYTORCH_PRETRAINED_BERT_CACHE),
                                                                                     'distributed')

    # Prepare model
    if args.gnn_model_type == 'transformer':
        model = GraphTransformer(caption_generator_state_dict=caption_generator_model_state_dict,
                                 caption_generator_cache_dir=caption_generator_cache_dir, args=args)
    elif args.gnn_model_type == 'gatv2':
        model = GraphGATv2(caption_generator_state_dict=caption_generator_model_state_dict,
                           caption_generator_cache_dir=caption_generator_cache_dir, args=args)
    else:  # gat
        model = GraphGAT(caption_generator_state_dict=caption_generator_model_state_dict,
                         caption_generator_cache_dir=caption_generator_cache_dir, args=args)
    model.to(device)
...
def prep_optimizer(args, theta_2_model, theta_1_model, num_train_optimization_steps, device, n_gpu, local_rank,
                   coef_lr=1.):
...
def train_epoch(epoch, args, theta_2_model, theta_1_model, train_dataloader, tokenizer, device, n_gpu, optimizer,
                scheduler,
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        decoder_scores_theta_1 = theta_1_model(data_geo_graph_batch, video_mask,
                                               input_caption_ids=pairs_input_caption_ids,
                                               decoder_mask=pairs_decoder_mask, batch_size=batch_sz, n_node=n_node)

        pairs_output_caption_ids = pairs_output_caption_ids.view(-1, pairs_output_caption_ids.shape[-1])

        loss_theta_2 = theta_2_model.module.decoder_loss_fct(
            decoder_scores_theta_2.view(-1, theta_2_model.module.bert_config.vocab_size),
            pairs_output_caption_ids.view(-1))

        loss_theta_1 = theta_1_model.module.caption_generator_model.decoder_loss_fct(
            decoder_scores_theta_1.view(-1, theta_1_model.module.caption_generator_model.bert_config.vocab_size),
            pairs_output_caption_ids.view(-1))
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if global_step % log_step == 0 and local_rank == 0:
                logger.info(
                    "Epoch: %d/%s, Step: %d/%d, Lr: %s, WD: %f, WS: %f, Loss: %f, Theta 2 Loss: %f, Theta 1 Loss: %f, Distill loss: %f, Time/step: %f",
                    epoch + 1,
                    args.epochs, step + 1,
...
def beam_decode_step(decoder, inst_dec_beams, len_dec_seq,
    ...
    def predict_word(next_decoder_ids, n_active_inst, n_bm, device, input_tuples):
        ...
        dec_output = decoder(visual_output_rpt, video_mask_rpt, next_decoder_ids, next_decoder_mask, shaped=True,
                             get_logits=True)
    ...
    def collect_active_inst_idx_list(inst_beams, word_prob, inst_idx_to_position_map, decoder_length=None):
        ...
        for inst_idx, inst_position in inst_idx_to_position_map.items():
            if decoder_length is None:
                ...
                is_inst_complete = inst_beams[inst_idx].advance(word_prob[inst_position],
                                                                word_length=decoder_length[inst_idx])
...
def eval_epoch(args, model, test_dataloader, tokenizer, device, n_gpu, nlgEvalObj=None, test_set=None):
            ...
            for len_dec_seq in range(1, args.max_words + 1):
                ...
                (visual_output_rpt, video_mask_rpt), \
                    inst_idx_to_position_map = collate_active_info((visual_output_rpt, video_mask_rpt),
                                                                   inst_idx_to_position_map, active_inst_idx_list, n_bm,
                                                                   device)
    ...
    logger.info(
        ">>>  METEOR: {:.4f}, ROUGE_L: {:.4f}, CIDEr: {:.4f}".format(metrics_nlg["METEOR"], metrics_nlg["ROUGE_L"],
                                                                     metrics_nlg["CIDEr"]))
