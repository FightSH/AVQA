...
def wav2fbank(self, filename, filename2=None, idx=None):
    # mixup
    if filename2 == None:
        waveform, sr = torchaudio.load(filename)
        waveform = waveform - waveform.mean()
...
def inference_dataloader(mode="visual"):
    ...
    for sample in samples:
        ...
        total_audio = []
        for audio_sec in range(10):
            fbank, mix_lambda = wav2fbank(os.path.join("./datasets/avqa/MUSIC_AVQA/audio", name + '.wav'),
                                                idx=audio_sec)
            total_audio.append(fbank)
        total_audio = torch.tensor(total_audio)

        sample = {'visual_posi': total_img, 'question': ques, 'label': label, 'audio': total_audio}
