...
class AverageMeter:
    ...
    def update(self, losses: List[Tuple[str, Tensor]], step_n=1):
        """更新平均计算器
        Args:
            losses: 损失名称和值的元组列表
            step_n: 步数
        ...
        for name, val in losses:
            ...
            if name not in self.data:
                self.data[name] = {'val': 0, 'avg': 0, 'sum': 0, 'count': 0}
            
            self.data[name]['val'] = val
            self.data[name]['sum'] += val * step_n
            self.data[name]['count'] += step_n
            self.data[name]['avg'] = self.data[name]['sum'] / self.data[name]['count']

    def get(self, key: str) -> float:
        """获取指定键的平均值"""
        if key in self.data:
            return self.data[key]['avg']
        return 0.0
...
def gather_losses(epoch: int, batch_idx: int, tot_batch: int, 
                 losses: List[Tuple[str, Tensor]], writer: SummaryWriter, 
                 device: torch.device) -> List[Tuple[str, Tensor]]:
    """收集和处理损失值
    Args:
        epoch: 当前轮次
        batch_idx: 当前批次索引
    ...
    """
    processed_losses = []
    global_step = epoch * (tot_batch + 1) + batch_idx
    ...
    for name, loss in losses:
        ...
        processed_losses.append((name, loss_value))
        
        # 写入TensorBoard
        if writer is not None:
            writer.add_scalar(f'train/{name}', loss_value, global_step)
    
    return processed_losses


def get_items(sample: Dict, device: torch.device) -> Dict:
    """将样本数据移动到指定设备并重新组织
    Args:
        sample: 原始样本数据
        device: 目标设备
    ...
    for key, value in sample.items():
        if isinstance(value, torch.Tensor):
            reshaped_data[key] = value.to(device)
        elif isinstance(value, list) and len(value) > 0 and isinstance(value[0], torch.Tensor):
            reshaped_data[key] = [v.to(device) for v in value]
        else:
...
def train(cfg: dict,
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        ce_loss = criterion(output['out'], target) if 'out' in output else criterion(output, target)
        loss += ce_loss
        ...
        if error_analyzer is not None:
            with torch.no_grad():
                model_output = output['out'] if 'out' in output else output
                ...
                if wrong_mask.any():
                    ...
                    for i in wrong_indices:
                        # 获取样本在数据集中的真实索引
                        dataset_idx = batch_idx * train_loader.batch_size + i
                        sample_info = train_loader.dataset.get_sample_info(dataset_idx)
                        ...
                        error_analyzer.record_error(
                            ...
                            predicted_logits=model_output[i],
...
def evaluate(cfg: dict,
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(val_loader):
            ...
            loss += criterion(model_output, target) / len(val_loader)
