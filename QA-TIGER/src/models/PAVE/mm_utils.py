...
def tokenizer_vision_token(prompt, tokenizer, vision_token=DEFAULT_IMAGE_TOKEN, image_token_index=IMAGE_TOKEN_INDEX, return_tensors=None): 
    ...
    input_ids = []
    offset = 0
    # for instance the llama tokenizer is always start with 1, then in this case we should skip the first one
    # when we concat all the sentence.
    if len(prompt_chunks) > 0 and len(prompt_chunks[0]) > 0 and prompt_chunks[0][0] == tokenizer.bos_token_id:
        offset = 1
        input_ids.append(prompt_chunks[0][0])

    # insert the -200 and concat all the idxes by ignoring all the first idx (which is 1)
...
def split_list_lengths(A, K):
    # Calculate the size of each part and the remainder
    avg = A // K
    remainder = A % K
    # Create a list where the first 'remainder' parts are of size 'avg + 1'
    lengths = [avg + 1 if i < remainder else avg for i in range(K)]
    return lengths
