...
def arg_parse() -> argparse.Namespace:
    ...
    parser.add_argument('--config', type=str, default='/mnt/sda/shenhao/code/AVQA/QA-TIGER/configs/qa_tiger/vitl14.py', help='Path to the config file')
    parser.add_argument('--distributed', action='store_true',
                        help='Use Distributed Data Parallel (DDP) if set; otherwise use Data Parallel (DP)')
...
def set_logger(cfg: dict) -> Tuple[SummaryWriter, logging.Logger, str, str]:
    ...
    if cfg.mode == 'test':
        ...
        if not cfg.debug:
            ...
            if dist.is_available() and ((dist.is_initialized() and dist.get_rank() == 0) or not dist.is_initialized()):
                logging_path.mkdir(parents=True, exist_ok=True)
                writer = SummaryWriter(logging_path.as_posix())
                logger = logging.getLogger(name="AVQA")
                logger.setLevel(logging.INFO)
...
def logging_config(config: dict) -> None:
    ...
    for i in range(torch.cuda.device_count()):
        ...
        logger.info(f"ㄴgpu device {i}: {name} - {mem}GB")