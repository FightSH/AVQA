...
该配置启用了反向对齐策略，以视频为锚点进行跨模态对齐。
特别适用于视频问答任务，能够保持视频信息的完整性。
"""
...
config = dict(
    ...
    num_labels=42,
    log_interval=100,
    output_dir='/mnt/sda/shenhao/code/AVQA/QA-TIGER/qa-tiger_clip_vitl14@336px_alignment_reverse',
    ...
    hyper_params=dict(
        gpus='1',
        model_type="QA-TIGER_ViTL14@336px_Alignment_Reverse",
        model=dict(
            d_model=512,
            video_dim=768,
            patch_dim=1024,
            ...
            alignment_config=dict(
                enabled=True,  # 启用对齐
                strategy='reverse',  # 反向对齐策略：以视频为锚点
                lambda_align=0.15,  # 稍高的对齐损失权重，因为视频信息更重要
                ot_eps=1e-8,  # 最优传输数值稳定性参数
                mmd_sigma=1.2,  # 稍大的MMD核参数，适应视频特征的复杂性
                patch_alignment=True,  # 启用patch特征对齐
                debug_mode=False,  # 生产环境关闭调试模式
                memory_efficient=True,  # 启用内存优化，视频序列通常较长
            )
        ),
        optim=dict(
            lr=1.5e-4,  # 稍低的学习率，因为对齐可能需要更稳定的训练
            encoder_lr=None,
            min_lr=1e-7,
            weight_decay=1e-2,
            betas=(0.95, 0.999)
        ...
        sched=dict(
            ...
            patience=5,
            verbose=True,
            warmup_epochs=3,  # 更长的warmup，适应对齐训练
        ),
    )
)