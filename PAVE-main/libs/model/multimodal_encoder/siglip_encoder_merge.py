...
class SigLipVisionTower(nn.Module):
    ...
    def load_model(self, device_map=None):
        ...
        self.vision_tower = SigLipVisionModel.from_pretrained(self.vision_tower_name, cache_dir='',device_map=device_map)
    ...
    def token_prune_merge_advanced(self, images, if_adaptive=True, reduction_ratio=1 / 8):
        ...
        # 设置hooks来提取最后一层的k和q
        # SigLip使用的是第26层（索引-1）
        hook_handle_k = self.vision_tower.vision_model.encoder.layers[-1].self_attn.k_proj.register_forward_hook(hook_k)
        hook_handle_q = self.vision_tower.vision_model.encoder.layers[-1].self_attn.q_proj.register_forward_hook(hook_q)

        # 前向传播
        image_forward_outs = self.vision_tower(images.to(device=self.device, dtype=self.dtype),
                                               output_hidden_states=True)
        image_features = image_forward_outs.hidden_states[-1].to(images.dtype)  # SigLip没有CLS token
        ...
        # SigLip没有CLS token，使用第一个patch的注意力
        first_patch_attn = attn[:, 0, :]

        if if_adaptive:
            reduction_ratio = outlier_detection(first_patch_attn)

        _, idx = torch.topk(first_patch_attn, int(N * reduction_ratio), dim=1, largest=True)
        index = idx.unsqueeze(-1).expand(-1, -1, C)

        x_others = torch.gather(image_features, dim=1, index=index)
        x_others_attn = torch.gather(first_patch_attn, dim=1, index=idx)
        Key_others = torch.gather(desired_layer_k, dim=1, index=index)
        ...
        non_topk_attn = torch.gather(first_patch_attn, dim=1, index=compl)
    ...
    def token_prune_merge_advanced_plus(self, images, if_adaptive=True, reduction_ratio=1 / 8):
        ...
        if if_adaptive:
            ...
            arithmetic_sequence = torch.arange(0, N, int(step_length / 3)).to(device=self.device)
            ...
            for i in range(idx.size(0)):
                arithmetic_sequence = torch.arange(int(step_length / 2), N, int(step_length)).to(device=self.device)
    ...
    def forward(self, images):
        if type(images) is list:
            ...
            for image in images:
                # 使用token merge
                image_feature = self.token_prune_merge_advanced_plus(image.unsqueeze(0))
                image_features.append(image_feature)
        else:
            # 使用token merge替代原始forward
