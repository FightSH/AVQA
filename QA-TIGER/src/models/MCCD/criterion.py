...
class MCCD_Criterion(nn.Module):
    def __init__(self, config, logger):
        super(MCCD_Criterion, self).__init__()
        self.config = config
        self.logger = logger
        self.c_fusion = nn.CrossEntropyLoss() # mean default

        # every loss weight
        self.major_loss_weight = float(self.config['loss_weight']['major_loss_weight'])
        self.distribution_loss_weight = float(self.config['loss_weight']['distribution_loss_weight'])
        self.euclidean_distance_fusion_q_weight = float(self.config['loss_weight']['euclidean_distance_fusion_q_weight'])
        self.euclidean_distance_fusion_a_weight = float(self.config['loss_weight']['euclidean_distance_fusion_a_weight'])
        self.euclidean_distance_fusion_v_weight = float(self.config['loss_weight']['euclidean_distance_fusion_v_weight'])

        self.cycle_Kl_loss_weight = float(self.config['loss_weight']['cycle_Kl_loss_weight'])
        self.cycle_KL_a_q_weight = float(self.config['loss_weight']['cycle_KL_a_q_weight'])
        self.cycle_KL_q_v_weight = float(self.config['loss_weight']['cycle_KL_q_v_weight'])
        self.cycle_KL_v_a_weight = float(self.config['loss_weight']['cycle_KL_v_a_weight'])

        self.cycle_KL_loss = None
        self.euclidean_distance = None
        if self.config['bias_learner']['three_bias_learner_exist']:
